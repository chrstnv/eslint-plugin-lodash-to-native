...
module.exports = {
  ...
  create: function(context) {
    ...
    /**
     * Функция возвращает строковое представление первого аргумента _.map()
     * @param {Object} node - объект узла с вызовом _.map()
     * @returns {string} - строковое представление первого аргумента _.map()
     */
    function getMapFirstArgumentAsString(node) {
      ...
      // вернуть строку с первым аргументом
      return nodeStr.substring(startA, endA);
    }
            ...
            if (allowedAncestors.includes(parentType)) {
              context.report({
                ...
                fix: function(fixer) {
                  return fixer.replaceText(
                    node,
                    `Array.isArray(${getMapFirstArgumentAsString(
                      node
                    )}) ? ${getNativeArrayMapExpression(node)} : ${nodeStr}`
                  );
                }
              });
            }
